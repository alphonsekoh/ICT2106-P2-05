...
namespace PainAssessment.Areas.Admin.Models.ModelBinder
{
    public class PractitionerModelBinder : IModelBinder
    {
        public Task BindModelAsync(ModelBindingContext bindingContext)
        {
            ...
            data.TryGetValue("SelectedPainEducation", out Microsoft.Extensions.Primitives.StringValues selectedPainEducation);
            ...
            if (nameResult)
            {
                string priorPainEducation = string.Join(",",selectedPainEducation);
                if (idResult)
                {
